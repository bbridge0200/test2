...
lint:
> $(PYTHON) -m flake8 --count --show-source --statistics test2
.PHONY: lint

type-check:
> $(PYTHON) -m mypy --ignore-missing-imports test2
.PHONY: type-check

fmt:
> $(PYTHON) -m black test2
.PHONY: fmt

import-sort:
> $(PYTHON) -m isort test2
.PHONY: import-sort

test:
> $(PYTHON) -m pytest test2/tests/
.PHONY: test
...
docs:
> source $(ACTIVATE) && sphinx-apidoc -f -o docs test2 test2/tests
